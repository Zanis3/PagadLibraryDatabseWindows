...
{
    public partial class UCAdminEditBook : UserControl
    {
        private UCAdminEditBookExtension editBookDetails;
        public UCAdminEditBook()
        {
            InitializeComponent();
        ...
        private void tblViewBooks_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                if (editBookDetails == null || editBookDetails.IsDisposed)
                {
                    editBookDetails = new UCAdminEditBookExtension(bookID);
                    editBookDetails.ShowDialog();
                }
                else
                {
                    editBookDetails.Close();
                    editBookDetails = new UCAdminEditBookExtension(bookID);
                    editBookDetails.ShowDialog();
                    editBookDetails.BringToFront();
                    editBookDetails.Focus();
                }
            }
        }
